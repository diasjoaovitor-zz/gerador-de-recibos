...
function salario(data) {
  const { salary, date, employee } = data
  const document = 'Recibo de Salário'
      ...
      },
      {
        table: {
          ...
          body: [
            [
              {
                text: `Declaro que recebi a quantia líquida de ${format.currency(salary)} - ${format.extensive(salary)}, referente ao pagamento do meu salário.`,
                margin: [10, 10, 10, 2],
