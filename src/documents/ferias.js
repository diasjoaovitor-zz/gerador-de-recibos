...
function ferias(data) {
  const { salary, oneThird, netValue, date, endDate, employee } = data

  const document = 'Recibo de Férias'
      ...
      },
      {
        table: {
          ...
          body: [
            [
              {
                text: `Declaro que recebi a quantia líquida de ${format.currency(netValue)} - ${format.extensive(netValue)}, referente ao pagamento das minhas férias.`,
                margin: [10, 10, 10, 2],
